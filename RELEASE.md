...
## 2026-06-29 — AlloyDB Terraform Module
- Created [GCP AlloyDB module](modules/database/gcp_alloyDB/README.md) supporting one or many [AlloyDB for PostgreSQL](https://cloud.google.com/alloydb/docs) clusters with `create` toggles, stable `key`-based `for_each`, and full configuration for primary instances (REGIONAL/ZONAL HA, vCPU sizing, database flags, columnar engine, SSL) and horizontally scalable read pool instances (`node_count` controls the number of parallel read nodes per pool).
- Added cluster-level controls for automated backups (count- or time-based retention, multi-day schedules), continuous backup for PITR (1–35 day recovery window), CMEK with customer-managed KMS keys, initial user provisioning, and maintenance window scheduling; all backup, PITR, and encryption blocks are dynamic so no null values are ever supplied to the provider.
- Published [AlloyDB deployment plan](tf-plans/gcp_alloydb/README.md) with wrapper files (`providers.tf`, `variables.tf`, `locals.tf`, `main.tf`, `outputs.tf`, `terraform.tfvars`) and commented examples covering a production HA cluster with read pool, a dev ZONAL single-instance cluster, and a disabled-toggle entry for safe staged rollout; the plan includes a Private Service Access (PSA) setup workflow and AlloyDB Auth Proxy connection instructions.
- Updated root documentation indexes and references in [README](README.md), [gcp-module-service-list](gcp-module-service-list.md), [gcp-service-list-definitions](gcp-service-list-definitions.md), [gcp-services-pricing-guide](gcp-services-pricing-guide.md), and [gcp-alloydb.md](modules/database/gcp_alloyDB/gcp-alloydb.md).
...
- Added [GCP VM deployment plan](tf-plans/gcp_vm/README.md) with three example entries: a public-facing Nginx web server, an internal database VM with a 500 GB SSD data disk, and a Spot batch worker with Shielded VM.
- Updated module count from 22 to 23; added module and plan links to all root markdown files.
